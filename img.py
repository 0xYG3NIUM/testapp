...
def get_surf_des(filename):
    ...
    #surf = cv2.xfeatures2d.SIFT_create(800)
...
class _img:
    ...
    def match(self, filename, limit=20):
        ...
        for i in range(0, len(matches)):
            match = matches[i]
            if match[0].distance < (.70 * match[1].distance):
                for img in self.imap:
...
class img:
    ...
    def match(self, filename, limit=20):
        ...
        sorted_sim = list(filter(lambda x:x['similarity'] > 5, sorted_sim))
        return sorted_sim
